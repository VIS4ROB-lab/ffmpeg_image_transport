...
static const ParameterDefinition params[] = {
    {ParameterValue("libx264"),
     ParameterDescriptor()
         ...
         .set__description(
             "ffmpeg encoder to use, see ffmpeg supported encoders")
    ...
    {ParameterValue(""),
     ParameterDescriptor()
         ...
         .set__type(ParameterType::PARAMETER_STRING)
         .set__description(
             "comma-separated list of AV options: profile:main,preset:ll")
         .set__read_only(false),
     ""},
    {ParameterValue(""),
     ParameterDescriptor()
     ...
     ParameterDescriptor()
         ...
         .set__integer_range(
             {rcl_interfaces::msg::IntegerRange()
                  ...
                  .set__to_value(std::numeric_limits<int>::max())
                  .set__step(1)}),
     "gop_size not set, defaulting to 10. This may severely affect lag and "
     "performance!"},
    {ParameterValue(static_cast<int>(30)),
     ParameterDescriptor()
         .set__name("frame_rate")
         .set__type(ParameterType::PARAMETER_INTEGER)
         .set__description("frame rate")
         .set__read_only(false)
         .set__integer_range({rcl_interfaces::msg::IntegerRange()
                                  .set__from_value(0)
                                  .set__to_value(60)
...
FFMPEGPublisher::FFMPEGPublisher()
    : logger_(rclcpp::get_logger("FFMPEGPublisher")) {}
...
void FFMPEGPublisher::declareParameter(NodeType node,
  ...
  if (n == "encoder") {
    ...
    RCLCPP_INFO_STREAM(logger_,
                       "using libav encoder: " << v.get<std::string>());
    ...
    encoder_.setFrameRate(v.get<int>(), 1);
...
void FFMPEGPublisher::handleAVOptions(const std::string& opt) {
  ...
  for (const auto& sl : split) {
    ...
    if (kv.size() != 2) {
      ...
      RCLCPP_INFO_STREAM(logger_,
                         "setting AV option " << kv[0] << " to " << kv[1]);
    }
  }
}

void FFMPEGPublisher::packetReady(const std::string& frame_id,
                                  const rclcpp::Time& stamp,
                                  const std::string& codec, uint32_t width,
                                  uint32_t height, uint64_t pts, uint8_t flags,
...
void FFMPEGPublisher::advertiseImpl(NodeType node,
...
FFMPEGPublisher::QoSType FFMPEGPublisher::initialize(
  ...
  uint ns_len =
      std::string(node.get_node_base_interface()->get_namespace()).length();
#else
...
void FFMPEGPublisher::publish(const Image& msg,
                              const PublisherTFn& publisher) const {
  FFMPEGPublisher* me = const_cast<FFMPEGPublisher*>(this);
  me->publishFunction_ = &publisher;
  if (!me->encoder_.isInitialized()) {
    if (!me->encoder_.initialize(msg.width, msg.height,
                                 std::bind(&FFMPEGPublisher::packetReady, me,
